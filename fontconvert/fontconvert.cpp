// fontconvert.cpp : Этот файл содержит функцию "main". Здесь начинается и заканчивается выполнение программы.
//
...
#define DPI 141 //141 // Approximate res. of Adafruit 2.8" TFT
...
int main(int argc, char* argv[]) {
    int i, j, err, size, first = ' ', last = '~', bitmapOffset = 0, x, y, byte, rusfont = 0, viewSymbol = 0,dpiX = 141,dpiY=141;
    char* fontName, c, * ptr;
    ...
    else if (argc >= 6)
    {
        first = atoi(argv[3]);
        last = atoi(argv[4]);
        rusfont = strcmp(argv[5], "RUS") == 0 && last > 192 ? 1 : 0;
    }

    if (argc == 7)
        dpiX = dpiY = atoi(argv[6]);
    else if (argc == 8)
    {
        dpiX = atoi(argv[6]);
        dpiY = atoi(argv[7]);
    ...
    strcpy_s(fontName, strlen(fontName), ptr);
    ...
    /*FT_WinFNT_Header winFNTHeader = NULL;
    err = FT_Get_WinFNT_Header(face, winFNTHeader);
    if (err)
        printf("Error %d get fnt header", err);*/

    err = FT_Select_Charmap(face, FT_ENCODING_UNICODE);
    if (err)
    {
        fprintf(stderr, "Error set unicode : %d", err);
        return err;
    }
    // << 6 because '26dot6' fixed-point format
    FT_Set_Char_Size(face, size << 6, 0, dpiX, dpiY);
    ...
    for (i = first, j = 0; i <= last; i++, j++) {
        ...
        if (rusfont && i >= 192 && i <= 255)
        {
            if ((err = FT_Load_Char(face, i + 848, FT_LOAD_TARGET_MONO))) {  //FT_LOAD_TARGET_MONO
                fprintf(stderr, "Error %d loading char '%c'\n", err, i);
                continue;
            }
        }
        else {
            if ((err = FT_Load_Char(face, i, FT_LOAD_TARGET_MONO))) {  //FT_LOAD_TARGET_MONO
                fprintf(stderr, "Error %d loading char '%c'\n", err, i);
                continue;
            }
        }
        ...
        for (y = 0; y < bitmap->rows; y++) {
            for (x = 0; x < bitmap->width; x++) {
                ...
                if (!viewSymbol)
                {
                    ...
                    //unsigned char c = gl->bitmap.buffer[y * gl->bitmap.width + x];
                }
                else {
                    if (bitmap->buffer[y * bitmap->pitch + byte] & bit)
                        putchar('#'); // Если ноль, записываем пробел
                    else putchar(' ');
                }
